...
/*************************************以下为校准部分**************************************************************************/
	...
	{ 
		vu16 crc_result;
		crc_result = (UART_Buffer_Rece[6] << 8) + UART_Buffer_Rece[7];
		if (crc_result == Hardware_CRC(UART_Buffer_Rece,6) )
		{
	...
	/*******************************内阻低端校准**********************************************/
			if(UART_Buffer_Rece[1] == 0x0F||flag_ADJ_VL==1)
			{
				Modify_A_READ = Rmon_value;//测量电压值
				Modify_A_ACT = (UART_Buffer_Rece[3] << 8) + UART_Buffer_Rece[4];//读取低段
			}
			if (UART_Buffer_Rece[1] == 0x20||flag_ADJ_VH==1)			   //电压测量校准完成
			{
				vu16 var16;
				vu32 var32a;
				vu32 var32b;
				...
				if(flag_OverV==1)//只有当有数据写入时才能将校准数据写入FLASH
				{
					...
					var16 = Modify_B_READ - Modify_A_READ;
					var32a = var32a / var16;
					REG_CorrectionR1 = var32a;
					var32a=0;
					var32a = Modify_B_ACT;
					var32a = var32a << 12;
					var32b = Modify_B_READ;
					var32b = var32b * REG_CorrectionR1;
					if (var32a < var32b)
					{
						var32b = var32b - var32a;
						REG_ReadR1_Offset = var32b;
						Polar3 |= 0x08;
					}
					else 
					{
						var32a = var32a - var32b;
						REG_ReadR1_Offset = var32a;
						Polar3 &= ~0x08;
					}
		//---------------------------------------------------------------------------------------//
					Flash_Write_all();	//参数写进FLASH
